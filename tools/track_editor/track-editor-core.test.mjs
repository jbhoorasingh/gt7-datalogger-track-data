...
// ── smoothing, the compiler's and the editor's ─────────────────────────────

const SMOOTH_VECTORS = JSON.parse(fs.readFileSync(path.join(HERE, "smooth-vectors.json"), "utf8"));

test("smoothRun gives the compiler's answer, to the micrometre, on the shared vectors", () => {
  // The file is generated by the datalogger's compiler. If this fails the two
  // routines have drifted, and the editor would show an admin a border the
  // published map will not have.
  assert.deepEqual(SMOOTH_VECTORS.parameters, {
    iterations: SMOOTH_ITERATIONS, lambda: SMOOTH_LAMBDA, mu: SMOOTH_MU, cap_m: SMOOTH_CAP_M,
  });
  const names = Object.keys(SMOOTH_VECTORS.cases);
  assert.ok(names.length >= 6);
  for (const name of names) {
    const { closed, points, expected } = SMOOTH_VECTORS.cases[name];
    const got = smoothRun(points.map(([x, z]) => ({ x, z })), { closed });
    assert.equal(got.length, expected.length, name);
    got.forEach((point, index) => {
      assert.ok(Math.abs(point.x - expected[index][0]) <= 1e-6, `${name}[${index}].x`);
      assert.ok(Math.abs(point.z - expected[index][1]) <= 1e-6, `${name}[${index}].z`);
    });
  }
});

test("smoothRun keeps a run's ends, honours the cap, and leaves its input alone", () => {
  const spiked = Array.from({ length: 20 }, (_, i) => ({ x: i, z: i === 10 ? 3 : 0 }));
  const before = JSON.stringify(spiked);
  const smoothed = smoothRun(spiked);
  assert.equal(JSON.stringify(spiked), before);
  assert.deepEqual(smoothed[0], spiked[0]);
  assert.deepEqual(smoothed.at(-1), spiked.at(-1));
  const moves = smoothed.map((p, i) => Math.hypot(p.x - spiked[i].x, p.z - spiked[i].z));
  assert.ok(Math.max(...moves) <= SMOOTH_CAP_M + 1e-9);
  assert.ok(Math.abs(moves[10] - SMOOTH_CAP_M) < 1e-9);
});

test("smoothRun does not cut a sharp corner the way an average does", () => {
  // A 3 m kerb through 90°, recorded on the 1 m grid. Judged against the true
  // border, not against the input: the input is already up to 0.6 m out.
  const radius = 3;
  const truth = [];
  for (let s = -20; s < 0; s += 0.05) truth.push([s, 0]);
  for (let a = 0; a < (Math.PI / 2) * radius; a += 0.05) {
    truth.push([radius * Math.sin(a / radius), radius * (1 - Math.cos(a / radius))]);
  }
  for (let s = 0; s < 20; s += 0.05) truth.push([radius, radius + s]);
  const worst = (points) => Math.max(...points.map((p) => Math.min(
    ...truth.map(([x, z]) => Math.hypot(p.x - x, p.z - z)),
  )));

  const recorded = SMOOTH_VECTORS.cases.kerb_r3.points.map(([x, z]) => ({ x, z }));
  const average = recorded.map((_, i) => {
    // One record a metre here, so ±8 records is the ±8 m window.
    const window = recorded.slice(Math.max(0, i - 8), i + 9);
    return {
      x: window.reduce((sum, p) => sum + p.x, 0) / window.length,
      z: window.reduce((sum, p) => sum + p.z, 0) / window.length,
    };
  });
  assert.ok(worst(smoothRun(recorded)) <= 0.4, `smoothRun is ${worst(smoothRun(recorded))} m out`);
  assert.ok(worst(average) > 1.0, `the average should be visibly worse, not ${worst(average)} m out`);
});

test("smoothRun smooths a loop round its seam and does not shrink it", () => {
  const n = 360;
  const ring = Array.from({ length: n }, (_, i) => {
    const r = 100 + 0.5 * (i % 2 ? -1 : 1);
    return { x: r * Math.cos((2 * Math.PI * i) / n), z: r * Math.sin((2 * Math.PI * i) / n) };
  });
  const area = (p) => Math.abs(p.reduce((sum, a, i) => {
    const b = p[(i + 1) % p.length];
    return sum + a.x * b.z - b.x * a.z;
  }, 0)) / 2;
  const smoothed = smoothRun(ring, { closed: true });
  assert.ok(Math.abs(area(smoothed) - area(ring)) / area(ring) < 0.001);
  const radii = smoothed.map((p) => Math.hypot(p.x, p.z));
  assert.ok(Math.max(...radii) - Math.min(...radii) < 0.2);
});
