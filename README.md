...
The compiler smooths each surveyed run of a border before it draws one — a
border as recorded steps sideways wherever the record kind changes — and the
service's administrator can switch that off: `compile.smooth_borders` in
Admin → Settings, served as `compile.smooth_borders` by `GET /v1/policy`. The
job passes the answer to every compile in the run, the published geometry and
its candidate alike, so "coverage held" compares like with like whichever way
it is set, and says which way in the first line of its log. The bundle on
`main` is never rewritten by it; off means the map is the evidence exactly as
recorded, from the next time each circuit is published. The editor carries the
same routine (`smoothRun` in `tools/track_editor/track-editor-core.mjs`) so it
can be run on its own, switch or no switch, and
`tools/track_editor/smooth-vectors.json` — generated by the datalogger's
`backend/tests/data/make_smooth_vectors.py` — is what holds the two to one
